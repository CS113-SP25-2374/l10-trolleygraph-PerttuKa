...
class TrolleyGraph {
    ...
    public List<String> dijkstra(String startStation, String endStation) {
        ...
        while (!pq.isEmpty()) {
            ...
            int currentIndex = getStationIndex(node.name);
            int currentDistance = distances[currentIndex];

            List<String> neighbors = getAdjacentStations(node.name);
            for (String neighbor : neighbors) {
                int neighborIndex = getStationIndex(neighbor);
                int neighborDistance = distances[neighborIndex];
                int routeWeight = getRouteWeight(node.name, neighbor);
                if(neighborDistance > currentDistance + routeWeight) {
                    distances[neighborIndex] = currentDistance +routeWeight;
                    parentMap.put(neighbor, node.name);
                    pq.add(new DNode(neighbor, distances[neighborIndex]));
                }
            }
        }
